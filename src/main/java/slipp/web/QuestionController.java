...
public class QuestionController {
	...
	public String create(Question newQuestion, HttpSession session) {
		...
		newQuestion.writeBy(sessionUser);
	...
	@PutMapping("/{id}")
	public String update(@PathVariable Long id, Question question, Model model, HttpSession session) {
		Question dbQuestion = questionRepository.findOne(id);
		Result result = valid(session, dbQuestion);
		if (!result.isValid()) {
			model.addAttribute("errorMessage", result.getErrorMessage());
			return "/user/login";
		}
